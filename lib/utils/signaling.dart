...
class Signaling {
  ...
  Future<void> createOffer(Session session) async {
    try {
      RTCSessionDescription offer = await session.pc!.createOffer(offerOptions);
  ...
  Future<void> createAnswer(Session session) async {
    try {
      RTCSessionDescription offer = await session.pc!.createAnswer();
      await session.pc!.setLocalDescription(_fixSdp(offer));
      Map<String, dynamic> offerMap = {
        'sdp': offer.sdp,
        'type': offer.type,
      };
      onSendMsg?.call(session.fromId, session.toId, 4, 5, json.encode(offerMap));
    } catch (e) {
      print(e.toString());
    }
  }
  ...
  //接通通话
  void accept(int fromId, String media) {
  ...
  void onReceive(WebSocketController webSocketController) {
    ...
    void _handleIceCandidate(Map<String, dynamic> msg) async {
      print("LIAO _handleIceCandidate");
      try {
        Session? session = _sessions[msg['fromId']];
        ...
        if (session != null) {
          ...
          _sessions[msg['fromId']] = Session(fromId: msg['fromId'], toId: msg['toId']);
          _sessions[msg['fromId']]?.remoteCandidates.add(candidate);
        }
      } catch (e) {
    ...
    //1-2、_handleOffer
    void _handleOffer(Map<String, dynamic> msg) async {
      print("LIAO _handleOffer");
      try {
        Session? session = _sessions[msg['fromId']];
        Session newSession = await createSession(session, msg['fromId'], msg['toId']);
        _sessions[msg['fromId']] = newSession;
        ...
        print("LIAO _handleOffer Error: $e");
      }
    }

    //1-3 _handleAnswer
    void _handleAnswer(Map<String, dynamic> msg) async {
